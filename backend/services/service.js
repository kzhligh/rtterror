const { getAllRecords, getRecordById, createRecord, updateRecord, deleteRecordById } = require('../daos/service');
...
async function getServiceById(serviceId) {
    if (typeof serviceId === 'string') {
        ...
        if (!service) {
            ...
            throw new Error(`ERROR - No such a service is found with the id: ${serviceId}`);
        }
...
async function createService(serviceObj) {
    const { id, name, description, treatment_type, duration, price, barcode, sms_description } = serviceObj;
    if (id && name &&description &&treatment_type &&duration &&price &&barcode && sms_description) {
...
async function updateService(serviceObj) {
    ...
    if (id && typeof id === 'string') {
        ...
        throw new Error('ERROR - no id has been specified in the input service object');
    }
}
