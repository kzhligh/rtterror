...
class EmployeeService extends GeneralService<IEmployee, IEmployeeDto> {
  ...
  async getItemById(id: string): Promise<IEmployee | null> {
    try {
      const employee = await this.model.findByPk(id, {
        include: this.serviceModel,
      });
      return (employee?.toJSON() ?? null) as IEmployee | null;
    } catch (error) {
   ...
   * */
  async createItem(itemInfo: IEmployeeDto): Promise<IEmployee | null> {
    const t = await sequelize.transaction();
    try {
      const { service_ids, ...employeeInfo } = itemInfo;
      const newEmployee = await this.model.create(employeeInfo, {
        transaction: t,
      });
      if (service_ids) {
  ...
  async updateItem(itemInfo: IEmployee): Promise<IEmployee | null> {
    const t = await sequelize.transaction();
    try {
      ...
      if (hasCreated)
        throw Error(`ERROR - no such an employee with id ${id} has been found`);
