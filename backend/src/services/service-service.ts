...
class ServiceService extends GeneralService<IService, IServicesDto> {
  ...
  constructor (serviceModelName: string, employeeModelName: string) {
  ...
  async createItems (itemInfo: IServicesDto): Promise<IService[]> {
    ...
    try {
      ...
      for (const durationsPrice of durations_prices) {
        ...
        const newService = await this.model.create(
          { ...serviceInfo, ...durationsPrice },
          { transaction: t }
        );
        if (employee_ids) {
        ...
        const savedService = (
          await this.model.findByPk(newService.getDataValue('id'), {
            include: this.employeeModel,
            transaction: t,
          })
        ).toJSON() as IService;
  ...
  async getItemById (id: string): Promise<IService> {
    try {
      return (
        await this.model.findByPk(id, { include: this.employeeModel })
      ).toJSON() as IService;
    } catch (error) {
  ...
  async updateItems (serviceObj: IServicesDto): Promise<IService[]> {
    ...
    try {
      ...
      // load employees
      const allEmployees: Model[] = [];
      for (const employeeId of employee_ids || []) {
        const employeeItem: Model = await this.employeeModel.findByPk(
          employeeId
        );
      ...
      for (const dp of durations_prices) {
        ...
        // without an id, it is a new service, add it
          ...
          const newService = await this.model.create(serviceInfo, {
            transaction: t,
          });
      ...
      await t.commit();
      const updatedServices = await this.model.findAll({
        where: {
          name: name,
          service_code: service_code,
          hidden: false,
  ...
  async blockUnblockServices (
    serviceCode: string,
    blockService: boolean
  ): Promise<IService[]> {
    const t = await sequelize.transaction();
    try {
      const [numberOfUpdates] = await this.model.update(
      ...
      if (numberOfUpdates === 0)
        throw new Error(
          `ERROR - no service of given service_code ${serviceCode} has been found`
        );
      await t.commit();
